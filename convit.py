...
from mask import get_mask_attn_sampling_bigbird ,get_mask_attn_shufle_dilated_each_layer , get_mask_attn_wythoff
...
class MHSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,depth_id=0):
        super().__init__()
        self.num_heads = num_heads
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads,  mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_gpsa=True,depth_id=0, **kwargs):
        super().__init__()
        ...
        if self.use_gpsa:
            ...
            self.attn = MHSA(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,depth_id=depth_id ,**kwargs)
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=48, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                use_gpsa=True,
                locality_strength=locality_strength,depth_id=i)
            if i<local_up_to_layer else
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                use_gpsa=False,depth_id=i)
